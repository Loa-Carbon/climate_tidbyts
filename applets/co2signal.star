...
USER_DATA_CACHE_EXPIRATION_SECONDS = 300 # 5 minute location cache
FONT = "tom-thumb"
            ...
            render.Row(
                ...
                children=[
                    render.Column(
                        ...
                        cross_align="center",
                        children=[
                            render.WrappedText(data["grid"],
                                               font=FONT),
                            render.WrappedText("%s %s" % (int(data["carbon_intensity"]), data["intensity_units"]),
                                               font=FONT),
                            render.WrappedText("fossil: %s%%" % fossil_fuel_percentage,
                                               font=FONT,
...
def get_data(api_key, location_string):
    ...
    if data == None:
        ...
        if response.status_code != 200:
            ...
            cache.set(user_cache_key, json.encode(data),
